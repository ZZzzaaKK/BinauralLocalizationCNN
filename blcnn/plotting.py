...
def main() -> None:
    ...
    if plotting_config.data_selection == "back" and plotting_config.folded == False:
        print(
            'Warning: Data selection is "back" but folded is False. Setting folded to True.'
        )
    ...
    for label in plotting_config.labels:
        ...
        for result_file in glob.glob(str(data_folder / "*.csv")):
            ...
            data = read_single_cnn_result(
                Path(result_file),
                plotting_config.data_selection,
                plotting_config.folded,
            )
            ...
            print(title)
            plt = plot_localization_accuracy(
                data,
            ...
            fig = plt.gcf()
            fig.set_size_inches(6 * 0.8, 3.5 * 0.8)
            plt.tight_layout()

            plt.savefig(
                data_folder
                / f"{Path(result_file).stem}{'_folded' if plotting_config.folded else ''}{'_binned' if plotting_config.binned else ''}.png",
                dpi=300,
            )
    ...
    for result_file in path.iterdir():
        with open(result_file, newline="") as csvfile:
            ...
            d = np.array(
                [
                    [
                        *CNNpos_to_loc_extra(int(row["true_class"])),
...
def read_single_cnn_result(path: Path, data_selection: str, folded: bool):
    with open(path, newline="") as csvfile:
        ...
        l = []
        for row in reader:
            true_class_loc = CNNpos_to_loc_extra(
                int(row["true_class"]), data_selection=data_selection, folded=folded
            )
            if (
                true_class_loc is None
            ):  # If the true class is not in the desired area, skip this row
                continue
            # For the predictions we don't want to filter out any values
            pred_class_loc = CNNpos_to_loc_extra(
                int(row["pred_class"]), data_selection="all", folded=folded
            )
    ...
    for azim in range(azimuth_min, azimuth_max + 1, azimuth_step):
        for elev in range(elevation_min, elevation_max + 1, elevation_step):
            for _ in range(nr_preds_per_speaker):
                azim_pred = (
                    azim + np.random.randint(-max_deviation, max_deviation + 1)
                ) * (1 - (center_skew * (abs(azim) * abs(elev)) / 10800))
                elev_pred = (
                    elev + np.random.randint(-max_deviation, max_deviation + 1)
                ) * (1 - (center_skew * (abs(azim) * abs(elev)) / 10800))
                data.append((azim, elev, azim_pred, elev_pred))
    return np.stack(data)


def plot_localization_accuracy(
    data,
    ...
    variability = np.mean(
        [
            np.std(responses[np.where(np.all(targets == target, axis=1))], axis=0)
    ...
    for azimuth in azimuths:
        for elevation in elevations:
            ...
            [perceived_targets] = data[
                np.where(
                    np.logical_and(data[:, 0] == azimuth, data[:, 1] == elevation)
                ),
                2:,
            ]
    ...
    axis.set_xlim(
        left=min_azim_pred - delta_azim_pred, right=max_azim_pred + delta_azim_pred
    )
    axis.set_ylim(
        bottom=min_elev_pred - delta_elev_pred, top=max_elev_pred + delta_elev_pred
    )
    ...
    if show_speaker_locs:
        axis.scatter(
            *np.unique(data[:, :2], axis=0).T, marker=",", s=1, linewidth=0, c="black"
        )
    ...
    axis.scatter(
        [min_azim, min_azim, max_azim, max_azim, min_azim],
        [min_elev, max_elev, max_elev, min_elev, min_elev],
        color=[
            colors(azim, elev, min_azim, max_azim, min_elev, max_elev)
            for azim, elev in [
                (min_azim, min_elev),
                (min_azim, max_elev),
                (max_azim, max_elev),
    ...
    for azim in azimuths:  # Vertical lines
        speaker_azims, speaker_elevs, pred_azims, pred_elevs = mean_predictions[
            mean_predictions[:, 0] == azim
        ].T
        ...
        axis.plot(
            speaker_azims,
            speaker_elevs,
            color=speaker_grid_color,
            linewidth=0.5,
            linestyle=speaker_grid_linestyle,
            alpha=0.5,
            zorder=1,
        )
    ...
    for single_azim, single_elev, single_azim_pred, single_elev_pred in data:
        ...
        if show_single_response_lines:
            ...
            mask = (mean_predictions[:, 0] == single_azim) & (
                mean_predictions[:, 1] == single_elev
            )
            [[mean_azim, mean_elev, mean_azim_pred, mean_elev_pred]] = mean_predictions[
                mask, :
            ]  # Unpack possible because it must be a single value
            axis.plot(
                [mean_azim_pred, single_azim_pred],
                [mean_elev_pred, single_elev_pred],
                color=colors(
                    mean_azim, mean_elev, min_azim, max_azim, min_elev, max_elev
                ),
                linestyle="--",
                linewidth=0.2,
                alpha=0.3,
        ...
        if show_single_responses:
            ...
            axis.scatter(
                ...
                single_elev_pred,
                c="None",
                edgecolors=single_response_color,
                linewidth=single_response_linewidth,
                s=single_response_size,
                zorder=1,
            )
...
def plot_model_diagram(model: keras.Sequential):
    ...
    # img = visualkeras.layered_view(model, legend=True)#, scale_xy=0.5, min_xy=20, scale_z=0.5)
    img = visualkeras.layered_view(
        model,
        legend=True,
        text_callable=_text_callable,
        font=ImageFont.load_default(size=36),
    )  # , scale_xy=0.5, min_xy=10, scale_z=0.5)
    # Remove boarder and ticks and everything
    plt.axis("off")

    # Make image large for printing
    fig = plt.gcf()
    fig.set_size_inches(10, 3)

    # Make figure span the whole window, top to bottom

    # Remove white borders
    plt.subplots_adjust(
        left=0.01, right=0.99, top=0.99, bottom=0.01, wspace=0, hspace=0
    )

    # Make high def for printing
    plt.rcParams["figure.dpi"] = 400

...
def main() -> None:
    inspect_data(
        Path(
            "data/cochleagrams/natural_selection_slab_kemar/test_cochleagrams.tfrecord"
        )
    )
...
def compare_datasets(path1: Path, path2: Path, plot: bool = True):
    ...
    if plot:
        ...
        for i in range(5):
            axs[i, 0].imshow(
                cochleagrams1[i][:, mid_start:mid_end, 1],
                aspect="auto",
                cmap="afmhot",
                origin="lower",
            )
            ...
            axs[i, 1].imshow(
                cochleagrams2[i][:, mid_start:mid_end, 1],
                aspect="auto",
                cmap="afmhot",
                origin="lower",
            )
    ...
    for i in range(5):
        print(
            f"Dataset 1 - Cochleagram {i + 1}: Min: {stats1[i][0]:.3f}, Max: {stats1[i][1]:.3f}, Avg: {stats1[i][2]:.3f}, RMS: {stats1[i][3]:.3f}"
        )
    print("---")
    for i in range(5):
        print(
            f"Dataset 2 - Cochleagram {i + 1}: Min: {stats2[i][0]:.3f}, Max: {stats2[i][1]:.3f}, Avg: {stats2[i][2]:.3f}, RMS: {stats2[i][3]:.3f}"
        )
...
def inspect_data(path: Path):
    ...
    print(
        f"Contains {dataset.cardinality()} examples (-1 is infinite cardinality, -2 is unknown cardinality)"
    )
    ...
    missing_elevs = [
        elev
        for elev in range(min(elevs), max(elevs) + 1, elev_step)
        if elev not in elevs
    ]
    ...
    missing_azims = [
        azim
        for azim in range(min(azims), max(azims) + 1, azim_step)
        if azim not in azims
    ]
    ...
    try:
        ...
        try:
            ...
            logger.error(
                "Could not reshape array, cochleagram should be 48000 or 8000 samples long"
            )
...
def extract_elev_azim(dataset: tf.data.TFRecordDataset) -> (List, List):
    ...
    for raw_record in tqdm(dataset, desc="Extracting elevations and azimuths"):
        ...
        azim, elev = CNNpos_to_loc(
            example.features.feature["train/target"].int64_list.value[0]
        )
                ...
                data = {
                    "train/image": tf.train.Feature(
                        bytes_list=tf.train.BytesList(
                            value=[
                                tf.compat.as_bytes(downsampled_cochleagram.tobytes())
                            ]
                        )
                    ),
        ...
        for tfrecord_path in tfrecord_paths:
            for record in tqdm(
                tf.data.TFRecordDataset(
                    tfrecord_path.as_posix(), compression_type="GZIP"
                ),
                unit="records",
                desc=f"Processing {tfrecord_path.name}",
            ):
...
def generate_pprint(
    ...
    explore_objects=True,
    excluded_ids=[],
    visited_ids=[],
    path_ids=[],
    current_depth=0,
    suppress_none_values=True,
):
    ...
    for key in keys:
        ...
        if isinstance(value, dict):
            valstr = (
                "<dict, %d items, class '%s'>" % (len(value), type(value).__name__)
                if (verbose_output == True)
                else "<dict, %d items>" % len(value)
            )
        elif isinstance(value, tuple):
            valstr = (
                "<tuple, %d items, class '%s'>" % (len(value), type(value).__name__)
                if (verbose_output == True)
                else "<tuple, %d items>" % len(value)
            )
        elif isinstance(value, list):
            valstr = (
                "<list, %d items, class '%s'>" % (len(value), type(value).__name__)
                if (verbose_output == True)
                else "<list, %d items>" % len(value)
            )
        elif isinstance(value, set):  # set and frozenset are distinct
            valstr = (
                "<set, %d items, class '%s'>" % (len(value), type(value).__name__)
                if (verbose_output == True)
                else "<set, %d items>" % len(value)
            )
        elif isinstance(value, frozenset):  # set and frozenset are distinct
            valstr = (
                "<frozenset, %d items, class '%s'>" % (len(value), type(value).__name__)
                if (verbose_output == True)
                else "<frozenset, %d items>" % len(value)
            )
        elif isinstance(value, range):
            valstr = (
                "<range, start %d, stop %d, step %d>"
                % (value.start, value.stop, value.step)
                if (verbose_output == True)
                else "<range(%d,%d,%d)>" % (value.start, value.stop, value.step)
            )
            ...
            valstr = (
                "<memoryview, %d bytes, object %s>"
                % (len(value), type(value.obj).__name__)
                if (verbose_output == True)
                else "<memoryview, %d bytes>" % len(value)
            )
        elif isinstance(
            value, bool
        ):  # needs to be above int as 'bool' also registers as 'int'
            valstr = "%s" % value
            ...
            value, type
        ):  # checks if object is 'class' (https://stackoverflow.com/a/10123520/1976617); needs to be above 'callable' as 'class' also registers as 'callable'
            # valstr = "<class '%s'>" % type(value).__name__ if (verbose_output == True) else "<class>"
            valstr = (
                "<class '%s.%s'>" % (value.__module__, value.__name__)
                if (verbose_output == True)
                else "<class>"
            )
        elif callable(
        ...
        if (
            ...
            or (isinstance(value, object) and exp_obj == True)
        ):
            ...
            #    continue
            recursion_output = generate_pprint(
                value,
                level_indent=level_indent,
                max_depth=max_depth,
                verbose_output=verbose_output,
                justify_output=justify_output,
                prevent_loops=prevent_loops,
    ...
    if prevent_loops == True:
        assert len(path_ids) > 0 and path_ids[-1] == id(obj), (
            "last item in list of path objects not existing or not matching object"
        )
        path_ids.pop()
...
def play_inverted_cochleagrams(path_to_cochleagrams):
    ...
    for coch in dataset:
        ...
        inv_signal_l, inv_coch_l = invert_cochleagram(
            np.array(coch[0])[:, :, 0],
            48000,
            37,
            30,
        ...
        inv_signal_r, inv_coch_r = invert_cochleagram(
            np.array(coch[0])[:, :, 1],
            48000,
            37,
            30,

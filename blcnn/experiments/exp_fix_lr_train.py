...
def main() -> None:
    ...
    Path(f"models/experiment_fixed_lr_trained_models").mkdir(
...
def run_experiment_fixed_lr(config):
    ...
    tensorboard_callback = keras.callbacks.TensorBoard(
        log_dir=(
            Path(
                f"models/experiment_fixed_lr_trained_models/{'_'.join(map(str, config))}/logs"
            )
        ).as_posix(),
        histogram_freq=0,
        write_graph=True,
    ...
    path_to_cochleagrams = Path("data/cochleagrams/natural_selection_slab_kemar")
    ...
    model: keras.Model = keras.models.load_model(
        Path("models/keras_momentum_9e-1/net1.keras"), compile=False
    )
    ...
    try:
        ...
        model_save_path = Path(
            f"models/experiment_fixed_lr_trained_models/{'_'.join(map(str, config))}/model.keras"
        )
